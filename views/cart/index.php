...
<div class="container">
    ...
    <?php if (!empty($cartItems)): ?>
        <div style="background-color: white; border-radius: 8px; padding: 20px; margin-top: 20px;">
                    ...
                    <tbody>
                        <?php foreach ($cartItems as $item): ?>
                            <tr style="border-bottom: 1px solid #ecf0f1;">
                                    ...
                                    <div style="display: flex; gap: 10px; align-items: center;">
                                        <div style="width: 60px; height: 60px; background-color: #ecf0f1; border-radius: 4px; display: flex; align-items: center; justify-content: center; overflow: hidden;">
                                            <?php if (!empty($item['product']['image'])): ?>
                                                <img src="public/images/<?php echo htmlspecialchars($item['product']['image']); ?>" style="width: 100%; height: 100%; object-fit: cover;">
                                            <?php else: ?>
                                        ...
                                        <div>
                                            ...
                                            <a href="?url=product/detail/<?php echo $item['product']['id']; ?>" style="color: #3498db; text-decoration: none; font-size: 12px;">Xem chi tiết</a>
                                        </div>
                                    </div>
                                </td>
                                ...
                                <td style="padding: 12px; text-align: center;">
                                    <input type="number" name="quantity[<?php echo $item['product']['id']; ?>]" value="<?php echo $item['quantity']; ?>" min="1" style="width: 60px; padding: 6px; border: 1px solid #bdc3c7; border-radius: 4px; text-align: center;">
                                </td>
                                <td style="padding: 12px; text-align: right; font-weight: bold; color: #e74c3c;">
                                    <?php echo number_format($item['subtotal'], 0, ',', '.'); ?> ₫
                                </td>
                                <td style="padding: 12px; text-align: center;">
                                    <a href="?url=cart/remove/<?php echo $item['product']['id']; ?>" class="btn btn-danger" style="padding: 6px 12px; font-size: 12px;">Xóa</a>
                                </td>
                            </tr>
                        <?php endforeach; ?>
                    </tbody>
            ...
            <div style="margin-top: 20px; display: flex; gap: 10px;">
                <a href="?url=product" class="btn btn-primary" style="padding: 12px 20px;">← Tiếp tục mua hàng</a>
                <a href="?url=cart/clear" class="btn btn-danger" style="padding: 12px 20px;">🗑️ Xóa tất cả</a>
    ...
    <?php else: ?>
        <div style="background-color: white; border-radius: 8px; padding: 40px; margin-top: 20px; text-align: center;">
            ...
            <a href="?url=product" class="btn btn-primary" style="padding: 12px 30px;">Tiếp tục mua hàng</a>
        </div>
    <?php endif; ?>
</div>
